...
For drawing *a polyline* or *a polygon* specify the firs point and continue clicking on the map. To stop drawing click the last point twice.
...
In fact, XYZ is a  OpenStreetMap standard, also known as https://wiki.openstreetmap.org/wiki/Slippy_map_tilenames[Slippy Map Tilenames], defining scheme for tiles that are served through a web server.
