...
1. Go to the user's home directory and create `.m2` folder if it doesn't exist. It is your local Maven repository. 

2. Unzip the file with artifacts into this directory. You should get the following directory structure:
+
image::structure.png[]
+
3. Double-click *Properties* in CUBA project tree. 
+
image::properties.png[]
+
4. Select the *Use local Maven repository* checkbox and apply the changes.

5. Go to *CUBA -> Marketplace* in the main menu.
+
image::main-menu.png[]
+
6. Click the *Gear* icon in the upper-right corner, then *Install by coordinates…*
+
image::gear.png[]
+
7. Enter coordinates of the latest add-on version. Copy coordinates from the *Component Coordinates* section on the page of the add-on on the https://www.cuba-platform.com/marketplace/[marketplace]. 
+
image::copy-coordinates.png[]
+
8. Add `.trial` at the end of the coordinates, for example:
+
image::coordinates1.png[]
+
9. Click *OK* and *Apply*. The installation will start.
