...
`ImageLayer` layer can be declared in the XML descriptor:
...
** `file:`  — the image will be served from file system.
+
[source]
----
...
* `topLeft` - the north-west point of an image (longitude, latitude).
* `bottomRight` - the south-east point of an image (longitude, latitude).
...
`HeatMapLayer` layer can be declared in the XML descriptor:
...
The layer's data can be provided in one of 2 different ways:
...
If the intensity value should be taken from some other property of number type (for example, `amount`), this property can be specified along with the data container:
+
[source,xml]
----
...
----
TIP: Intensity value of a point is in range between 0 and 1 by default. You can change this range by setting different maximum intensity value in the `maximumIntensity` property.
...
[source,xml]
----
<maps:heatmap id="heatmap" dataContainer="ownersDc">
    <maps:gradient>
